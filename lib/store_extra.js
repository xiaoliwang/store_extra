...
;(function(root,factory){
    ...
    if(isStorageSupported()){
        ...
        store.set = function(key, val) {
            if(val == null) return store.remove(key);
        ...
        //监听key的变化（不支持ie8）
        store.onKey = function(key,callback){
            var eventFun1 = function(e) {
                if (e.key === key){
                    callback(store.deserialize(e.newValue));
                }
            };
        ...
        //取消事件
        store.off = function(eventId){
            var event = store.eventFuns[eventId];
            if(!('onstorage' in doc)) {
